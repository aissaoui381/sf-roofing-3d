...
const STEPS = [
  {
    ...
    options: [
      { id: 'repair',       emoji: '🔧', label: 'Roof Repair',       desc: 'Fix leaks or damage',    base: { min: 500,   max: 2500  }, color: 'hover:border-orange-400/60 hover:bg-orange-500/[0.06]' },
      { id: 'replacement',  emoji: '🏠', label: 'Full Replacement',   desc: 'Complete new roof',      base: { min: 8000,  max: 18000 }, color: 'hover:border-amber-400/60  hover:bg-amber-500/[0.06]'  },
      { id: 'installation', emoji: '🏗️', label: 'New Construction',   desc: 'New build roofing',      base: { min: 10000, max: 22000 }, color: 'hover:border-yellow-400/60 hover:bg-yellow-500/[0.06]' },
      { id: 'inspection',   emoji: '🔍', label: 'Inspection Only',    desc: 'Assessment & report',    base: { min: 250,   max: 550   }, color: 'hover:border-sky-400/60    hover:bg-sky-500/[0.06]'    },
    ],
  },
  {
    id: 'size', title: 'Approximate roof size?',
    options: [
      { id: 'small',  emoji: '📐', label: 'Small',       desc: 'Under 1,000 sq ft',  factor: 0.7, color: 'hover:border-green-400/60  hover:bg-green-500/[0.06]'  },
      { id: 'medium', emoji: '📏', label: 'Medium',      desc: '1,000–2,000 sq ft',  factor: 1,   color: 'hover:border-amber-400/60  hover:bg-amber-500/[0.06]'  },
      { id: 'large',  emoji: '🏛️', label: 'Large',       desc: '2,000–3,500 sq ft',  factor: 1.6, color: 'hover:border-orange-400/60 hover:bg-orange-500/[0.06]' },
      { id: 'xlarge', emoji: '🏢', label: 'Extra Large', desc: 'Over 3,500 sq ft',   factor: 2.4, color: 'hover:border-rose-400/60   hover:bg-rose-500/[0.06]'   },
    ],
  },
  {
    id: 'material', title: 'Preferred material?',
    options: [
      { id: 'asphalt', emoji: '▪️', label: 'Asphalt Shingles', desc: '20–30 yr lifespan', factor: 1,   color: 'hover:border-slate-400/60  hover:bg-slate-500/[0.06]'  },
      { id: 'metal',   emoji: '⚙️', label: 'Metal Roofing',    desc: '40–70 yr lifespan', factor: 1.8, color: 'hover:border-zinc-400/60   hover:bg-zinc-400/[0.06]'   },
      { id: 'tile',    emoji: '🪨', label: 'Tile / Slate',      desc: '50+ yr lifespan',   factor: 2.2, color: 'hover:border-stone-400/60  hover:bg-stone-500/[0.06]'  },
      { id: 'flat',    emoji: '⬜', label: 'Flat / TPO',        desc: '15–25 yr lifespan', factor: 1.3, color: 'hover:border-sky-400/60    hover:bg-sky-500/[0.06]'    },
    ],
  },
  {
    id: 'timeline', title: 'When do you need it?',
    options: [
      { id: 'urgent',   emoji: '🚨', label: 'As Soon As Possible', desc: 'Within the week',   factor: 1.2,  color: 'hover:border-red-400/60    hover:bg-red-500/[0.06]'    },
      { id: 'month',    emoji: '📅', label: 'Within a Month',       desc: 'Standard schedule', factor: 1,    color: 'hover:border-amber-400/60  hover:bg-amber-500/[0.06]'  },
      { id: 'quarter',  emoji: '🗓️', label: 'Next 3 Months',        desc: 'Flexible timing',   factor: 0.95, color: 'hover:border-green-400/60  hover:bg-green-500/[0.06]'  },
      { id: 'planning', emoji: '💭', label: 'Just Planning',         desc: 'No rush',           factor: 0.9,  color: 'hover:border-sky-400/60    hover:bg-sky-500/[0.06]'    },
    ],
  },
];
...
export default function QuoteCalculator() {
  ...
  return (
    <section
      id="quote"
      ref={containerRef}
      className="py-16 px-6 md:px-16 lg:px-24 bg-zinc-900 relative overflow-hidden"
    >
      {/* Radial glow */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{ background: 'radial-gradient(ellipse 70% 60% at 50% 0%, rgba(206,152,67,0.12) 0%, transparent 70%)' }}
      />
      ...
      <div className="max-w-7xl mx-auto relative">
          ...
          <div className="flex flex-col gap-4 md:mb-2">
            ...
            <div className="flex flex-wrap gap-2">
              {['No commitment', 'No spam', 'Instant results'].map((t) => (
                <span key={t} className="inline-flex items-center gap-1.5 text-xs text-zinc-400 bg-zinc-800 border border-zinc-700 rounded-full px-3 py-1">
        ...
        <div className="calc-body">
                ...
                return (
                  <div key={s.id} className="flex items-center flex-1 last:flex-none">
                    <div className={`
                      ...
                      ${!done && !current ? 'bg-zinc-800 border-zinc-700 text-zinc-500'                        : ''}
                    `}>
                      {done ? <Check size={16} strokeWidth={3} /> : i + 1}
                    </div>
                    {i < STEPS.length - 1 && (
                      <div className={`flex-1 h-0.5 mx-2 rounded-full transition-all duration-500 ${step > i ? 'bg-gold' : 'bg-zinc-800'}`} />
                    )}
                  </div>
                );
              })}
          ...
          {/* Main card — wide */}
          <div className="rounded-2xl bg-zinc-950/80 border border-zinc-800 backdrop-blur-sm shadow-[0_24px_80px_rgba(0,0,0,0.6)]">
            {submitted ? (
              ...
              <div className="p-12 md:p-16 text-center">
                ...
                <h3 className="text-3xl font-black text-white mb-4">Estimate on its way!</h3>
                ...
                {step < STEPS.length ? (
                  ...
                  <div className="p-8 md:p-10 lg:p-12">
                    <div className="flex items-start justify-between mb-8">
                      <div>
                        <p className="text-zinc-500 text-sm font-medium mb-2">Step {step + 1} of {STEPS.length}</p>
                        <h3 className="text-2xl md:text-3xl font-black text-white">{STEPS[step].title}</h3>
                      </div>
                      {step > 0 && (
                        <button
                        ...
                        return (
                          <button
                            ...
                            className={`
                              ...
                              ${isSelected
                                ? 'bg-gold/[0.12] border-gold shadow-[0_0_24px_rgba(206,152,67,0.18)] scale-[1.02]'
                                : `bg-zinc-900 border-zinc-700/70 ${opt.color} active:scale-[0.98]`
                              }
                            `}
                          >
                            {/* Selected accent bar */}
                            ...
                            <div className="flex-1">
                              <div className={`font-bold text-base leading-tight mb-1 ${isSelected ? 'text-gold' : 'text-white'}`}>
                                {opt.label}
                              </div>
                  ...
                  <div className="p-8 md:p-12 lg:p-14">
                    <div className="grid md:grid-cols-2 gap-10 md:gap-16 items-start">
                      <div>
                        ...
                        <h3 className="text-2xl font-black text-white mb-3">Almost there!</h3>
                      ...
                      <div className="space-y-3">
                        ...
                        <input
                          ...
                          required
                          className="w-full px-5 py-4 bg-zinc-800 border-2 border-zinc-700 rounded-xl
                        ...
                        <p className="text-center text-zinc-600 text-xs pt-1">No spam. Unsubscribe anytime.</p>
                      </div>
                    </div>
                  </div>
