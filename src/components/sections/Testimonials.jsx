...
export default function Testimonials() {
  ...
  return (
    ...
    >
      {/* Background accent */}
      <div className="absolute top-0 right-0 w-64 h-64 md:w-96 md:h-96 bg-gold/[0.06] rounded-full blur-3xl translate-x-1/3 -translate-y-1/3 pointer-events-none" />
        ...
        <div className="grid md:grid-cols-[280px_1fr] gap-14 md:gap-20 items-start">
          ...
          <div className="flex flex-col gap-10">
            <div>
              ...
              <h2 className="text-4xl md:text-5xl font-black text-zinc-900 leading-[1.0] tracking-tight">
                What<br />Homeowners<br />Say
              </h2>
            </div>

            <div>
              <p className="text-4xl font-black text-zinc-900 tabular-nums mb-4">
                <span className="text-gold">{String(index + 1).padStart(2, '0')}</span>
                <span className="text-zinc-300 text-2xl"> / </span>
                <span className="text-zinc-300">{String(TESTIMONIALS.length).padStart(2, '0')}</span>
              </p>

              <div className="flex gap-2">
                <button
                  onClick={prev}
                  className="w-10 h-10 rounded-full border border-zinc-300 hover:border-[#DD9E3A]/50 hover:bg-[#DD9E3A]/[0.08]
                ...
                {TESTIMONIALS.map((_, i) => (
                  <button
                    ...
                    className={`h-1 rounded-full transition-all duration-300 ${
                      i === index ? 'w-8 bg-[#DD9E3A]' : 'w-2 bg-zinc-300 hover:bg-zinc-400'
                    }`}
                  />
                ))}
              </div>
            ...
            >
              {TESTIMONIALS.map((t, i) => (
                <div key={i} className="w-full min-w-full flex-shrink-0 relative pr-1">
                  ...
                  <div
                    className="absolute -top-8 -left-4 text-[10rem] md:text-[14rem] font-serif leading-none text-[#DD9E3A]/[0.1] select-none pointer-events-none"
                    aria-hidden
                  >
                    "
                  </div>
                  ...
                  <div className="flex gap-1 mb-6 relative">
                    {Array.from({ length: 5 }).map((_, si) => (
                      <Star key={si} className="w-5 h-5 fill-[#DD9E3A] text-[#DD9E3A]" />
                    ))}
                  </div>

                  {/* Quote */}
                  <p className="text-zinc-700 text-xl md:text-2xl leading-relaxed font-light mb-8 relative">
                    {t.text}
                  </p>
                  ...
                  <div className="flex items-center gap-4 relative">
                    ...
                    <div>
                      ...
                      <div className="text-zinc-400 text-sm mt-0.5">{t.role}</div>
                    </div>
                  </div>

                  {/* Auto-progress bar */}
                  <div className="mt-8 h-px bg-zinc-200 rounded-full overflow-hidden">
                    <div
                      key={`tpb-${index}`}
